...
install_kes() {
    ...
    echo "Creating ${INIT_STORAGE_KES_CERT_KEY_FILE}, ${INIT_STORAGE_KES_CERT_FILE}" "${INIT_STORAGE_KES_CERT_ID_FILE}"
    touch "${INIT_STORAGE_KES_CERT_KEY_FILE}" "${INIT_STORAGE_KES_CERT_FILE}" "${INIT_STORAGE_KES_CERT_ID_FILE}"
    chown "${INIT_STORAGE_KES_USER}:${INIT_STORAGE_KES_GROUP}" \
        "${INIT_STORAGE_KES_CERT_KEY_FILE}" "${INIT_STORAGE_KES_CERT_FILE}" "${INIT_STORAGE_KES_CERT_ID_FILE}"
    ...
    docker run --rm --user "$(id "${INIT_STORAGE_KES_USER}" -u):$(id "${INIT_STORAGE_KES_GROUP}" -g)" \
        -v "${INIT_STORAGE_KES_CERT_FILE}":"${INIT_STORAGE_KES_CERT_FILE}" \
        -v "${INIT_STORAGE_KES_CERT_ID_FILE}":"${INIT_STORAGE_KES_CERT_ID_FILE}" \
        --name "${INIT_STORAGE_KES_CONTAINER_NAME}" --entrypoint "" \
        "minio/kes:${INIT_STORAGE_KES_VERSION}" /bin/bash -c "/kes identity of ${INIT_STORAGE_KES_CERT_FILE} |
        tee ${INIT_STORAGE_KES_CERT_ID_FILE}"

    INIT_STORAGE_KES_ADMIN_IDENTITY="$(cat "${INIT_STORAGE_KES_CERT_ID_FILE}")"

    if [[ -z "${INIT_STORAGE_KES_ADMIN_IDENTITY}" ]] || [[ "${INIT_STORAGE_KES_ADMIN_IDENTITY}" == "" ]]; then
        echo "ERROR:: INIT_STORAGE_KES_ADMIN_IDENTITY is empty"
        exit 1
    fi
...
export INIT_STORAGE_DIR="${INIT_STORAGE_DIR:-"/app/sdic/minio"}"

export INIT_STORAGE_DOCKER_NETWORK="${INIT_STORAGE_DOCKER_NETWORK:-"sdic-minio"}"

export INIT_STORAGE_KES_WORKDIR="${INIT_STORAGE_KES_WORKDIR:-"${INIT_STORAGE_DIR}/kes"}"

export INIT_STORAGE_KES_VERSION="${INIT_STORAGE_KES_VERSION:-"2024-06-17T15-47-05Z"}"
...
export INIT_STORAGE_KES_GROUP="${INIT_STORAGE_KES_GROUP:-"sdic-minio-kes"}"

export INIT_STORAGE_KES_USER="${INIT_STORAGE_KES_USER:-"sdic-minio-kes"}"

export INIT_STORAGE_KES_CONTAINER_NAME="${INIT_STORAGE_KES_CONTAINER_NAME:-"sdic-minio-kes"}"
...
export INIT_STORAGE_MINIO_VERSION="${INIT_STORAGE_MINIO_VERSION:-"RELEASE.2024-06-29T01-20-47Z"}"

export INIT_STORAGE_MINIO_CONTAINER_NAME="${INIT_STORAGE_MINIO_CONTAINER_NAME:-"sdic-minio-minio"}"

export INIT_STORAGE_MINIO_GROUP="${INIT_STORAGE_MINIO_GROUP:-"sdic-minio-minio"}"

export INIT_STORAGE_MINIO_USER="${INIT_STORAGE_MINIO_USER:-"sdic-minio-minio"}"
...
export INIT_STORAGE_MINIO_KMS_KES_KEY_NAME="${INIT_STORAGE_MINIO_KMS_KES_KEY_NAME:-"sdic-minio-default"}"
